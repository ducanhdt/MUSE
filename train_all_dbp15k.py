...
def train_all():
    ...
    for lang_pair in lang_pairs:
        ...
        for embeding_type in ['mbert', 'xlm-r','me5']:
            ...
            command = f"python {method}.py --exp_name {method}_{src_lang}_{tgt_lang}_{embeding_type}_{train_size} --exp_path dumped_dbp15 --exp_id 0 --src_lang {src_lang} --tgt_lang {tgt_lang} --emb_dim 768 --n_refinement 5 --dico_eval {test_path} --src_emb {src_emb} --tgt_emb {tgt_emb} --export pth --dis_most_frequent 0"
            os.system(command)
            end = time.time()
            log = '\t'.join([method, src_lang, tgt_lang, embeding_type, str(train_size), str(end - start)])
            with open("time_dbp15.txt", "a") as f:
                f.write(f"{log}\n")
    ...
    for lang_pair in lang_pairs:
        ...
        for train_size in [30, 50, 70, 90]:
            ...
            for embeding_type in ['mbert', 'xlm-r', 'me5']:
                ...
                src_emb = f"{embedding_path}/{lang_pair}/{src_lang}_{embeding_type}.pth"
                tgt_emb = f"{embedding_path}/{lang_pair}/{tgt_lang}_{embeding_type}.pth"
                command = f"python {method}.py --exp_name {method}_{src_lang}_{tgt_lang}_{embeding_type}_{train_size} --exp_path dumped_dbp15 --exp_id 0 --src_lang {src_lang} --tgt_lang {tgt_lang} --emb_dim 768 --n_refinement 5 --dico_train {train_path} --dico_eval {test_path} --src_emb {src_emb} --tgt_emb {tgt_emb} --export pth "
                print(command)
                os.system(command)
                end = time.time()
                log = '\t'.join([method, src_lang, tgt_lang, embeding_type, str(train_size), str(end - start)])
...
if __name__ == "__main__":
    with open("time_dbp15.txt", "w") as f:
